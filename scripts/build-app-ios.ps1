# Script para crear build de iOS con EAS

Write-Host "🍎 Creando build de iOS..." -ForegroundColor Cyan
...
Write-Host "1. Preview (IPA - para probar)" -ForegroundColor White
Write-Host "2. Production (IPA - para App Store)" -ForegroundColor White
$choice = Read-Host "Selecciona (1 o 2)"

if ($choice -eq "1") {
    Write-Host "📱 Creando build Preview (IPA)..." -ForegroundColor Yellow
    eas build --platform ios --profile preview
} elseif ($choice -eq "2") {
    Write-Host "📱 Creando build Production (IPA)..." -ForegroundColor Yellow
